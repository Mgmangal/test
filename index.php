...
public function setQuotationByBA() {
			...
			$data['bank_data'] = $this->db->select('users.*,user_details.cug_mobile')->from('users')->join('user_details', 'user_details.user_id = users.id')->where(array('users.id'=>$get_case_data1['added_by']))->get()->row_array(); 
			$data['created_by_name'] = $this->db->select('name')->from('users')->where(array('id'=>$created_by))->get()->row_array();
			...
			$data['report_preparation_data'] = $this->db->select('*')->from('report_prepare_master')->where(array('case_id'=>$lead_com_id,'user_role_type'=>3,'status'=>3))->get()->row_array();
			...
			{ 
					...
					if($page_id == 5)
					{
					   //  echo $lead_id;
							...
							if($no_advance_status == 2){ // approved
								if($no_advance_quo_purpose == 1){ // notify only
									...
									$vars = array(
										...
										'[$Coordinating_Person]'	=> ucwords($get_case_data1['co_persone_name']),
		                              	...
		                              	'[$AMOUNT]'		=> curency_formate($quotation),
		                              	'[$USER_NAME]' 	=> $ba_name,
									...
									$vars1 = array('[$CASE_ID]'=> $vis_case_id,'[$Customer_Name]'=>ucwords($get_case_data1['customer_name'])); 
									$sub = strtr($getEmail['subject'], $vars1);
									$get_leads_data = $this->db->select('*')->from('leads')->where(array('id' => $lead_id))->get()->row_array();
									$head_e_id = $get_case_data1['customer_email_id'];
									$head_e_id2 = $get_case_data1['co_persone_email'];
									
									$this->email->cc($bank_email.','.$ba_email);
									...
									$vars2 = array('[$CASE_ID]'=> $vis_case_id,'[$Customer_Name]'=> ucwords($get_case_data1['customer_name'])); 
								...
								$vars = array(
									...
									'[$Coordinating_Person]'	=> ucwords($get_case_data1['co_persone_name']),
	                              	...
	                              	'[$Bank_Name]'			=> ucwords($bank_Det['bank_name']),
                  					'[$Manager_Name]'		=> ucwords($bank_Det['name']),
	                              	'[$AMOUNT]'		=> curency_formate($due_quotation_val),
	                              	'[$QUOTATION]'		=> $quotation,
	                              	'[$USER_NAME]' 	=> $ba_name,
	                              	'[$PAYMENT_TERM]'=>$no_advance_pay_term_text,
                    				'[$USER_Official_PHONE]' => $ba_mobile,
						...
						if($quotation_type == 1){ // for custom quotation
							...
							$n_cond = array('status'=> 1,'emp_id'=> $userId,'res_id'=> $lead_id,'notification_type'	=> 8);
		        			$this->db->where($n_cond)->update('user_notification', $n_arr);
						} else { // for as per bank fee
							// clear DGM VIS Notification
							$userId = $this->session->userdata('adminData')['userId'];
							$n_arr = array('status'=> '2','updated_at'	=> date('Y-m-d H:i:s'));
							$n_cond = array('status'	=> 1,
						    				// 'emp_id'	=> $userId,
						    				'res_id'	=> $lead_id,
						    				'notification_type'	=> 8
						    				);
	 		          ...
	 		          if($due_quotation_val != '0.00'){
						...
						//	print_r($arr);die;
							...
							foreach ($dgm_emp as $key => $value) 
							{
						        ...
						        // send email
						        $dgm_emp_detail = $this->db->select('*')->from('users')->where(array('id'=>$value['user_id']))->get()->row_array();
								$ba_id = $get_view_data['assign_lead'];
								$ba_Det = $this->db->select('*')->from('users')->where(array('id'=>$ba_id))->get()->row_array();
				                $ba_name = ucwords($ba_Det['name']);
				                $ba_email = ucwords($ba_Det['email']);
								...
								$getEmail = $this->w_user->getEmail('notification_setting_master', $cond_1);
								$vars1 = array('[$CASE_ID]'=> $get_view_data['lead_ids']); 
			        			$sub = strtr($getEmail['subject'], $vars1);
				...
				if (!empty($per_id)) { 
					$get_percent_name = $this->db->select('*')->from('payment_value')->where(array('status' => '1', 'id' => $per_id))->get()->row_array();
				...
				if (!empty($lead_com_id)) {
					$data['data'] = $this->db->select('*')->from('lead_component')->where('id', $lead_com_id)->get()->row_array();
				} else {
